...
void cb::VisionProcessor::Periodic()
{
  ...
  while (m_runningCamera)
  {
    ...
    if (m_coneProcessor.GetFindBlobsOutput()->size() > 0)
    { // if we have detected shapes
      std::cout << "Shapes detected!\n";
      std::vector<std::vector<cv::Point>> contours;
      for (const cv::KeyPoint &blob : *blobs)
      { // convert the blobs to contours
        contours.push_back(std::vector<cv::Point>{blob.pt});
      }
      cv::Rect r = cv::boundingRect(contours);
      m_centerX = r.x + (r.width / 2); // update centerX
    }
  }
}

std::pair<units::meter_t, units::radian_t> cb::VisionProcessor::getObjectPosition()
{
 return {};
}
